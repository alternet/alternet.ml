...
Note in the example the `PlainTextCryptFormat` is used as the last attempt to check clear passwords, and MUST NOT be used in production. The [`ModularCryptFormat`](../security-auth/apidocs/ml/alternet/security/auth/formats/ModularCryptFormat.html)) is certainly the better choice nowaday, and if the crypt belongs to this family, the relevant hasher will be get from [`ModularCryptFormatHashers`](../security-auth/apidocs/ml/alternet/security/auth/formats/ModularCryptFormat.Hashers.html) - [source code](https://github.com/alternet/alternet.ml/blob/master/security-auth/src/main/java/ml/alternet/security/auth/formats/ModularCryptFormat.java#L102)
