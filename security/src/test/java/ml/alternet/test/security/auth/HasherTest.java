...
public class HasherTest {
    ...
    static final char[] PASSWORD = "Da_actu@L pazzw0r|) !".toCharArray();
    ...
    public void PBKDF2Hasher_Should_RejectUnknownProperty() throws InvalidAlgorithmParameterException {
        ...
        Hasher h = HashUtil.lookup(CryptFormat.COLON_CRYPT_FORMAT.family(), "PBKDF2", prop).get();
    ...
    public void PBKDF2Crypt_Should_HaveCorrectForm() throws InvalidAlgorithmParameterException {
        ...
        Integer iter = (Integer) h.getConfiguration().get(PBKDF2Hasher.ITERATIONS_PROPERTY_NAME);
        Assertions.assertThat(Integer.parseInt(parts[1])).isEqualTo(iter);
        Assertions.assertThat(parts[3].toCharArray()).isNotEqualTo(PASSWORD);
    }
    ...
    public void PBKDF2Crypt_Should_MatchPassword(String crypt) throws InvalidAlgorithmParameterException {
        Hasher h = CryptFormat.COLON_CRYPT_FORMAT.resolve(crypt)
                .orElseThrow(InvalidAlgorithmParameterException::new);
        Assertions.assertThat(h.check(null, PASSWORD, crypt)).isTrue();
    }

    @Test
    public void PBKDF2Crypt_Should_GenerateDifferentCrypts() throws InvalidAlgorithmParameterException {
        Hasher h = Hasher.getDefault();
        String c1 = h.encrypt(PASSWORD);
        String c2 = h.encrypt(PASSWORD);
        String c3 = h.encrypt(PASSWORD);
        String c4 = h.encrypt(PASSWORD);
        ...
        Assertions.assertThat(crypts).are(new Condition<String>() {
            ...
            public boolean matches(String value) {
                try {
                    return h.check(null, PASSWORD, value);
                } catch (InvalidAlgorithmParameterException e) {
                    throw new RuntimeException(e);
                }
            }
        });
        //        crypts.stream().forEach(s ->
        //            {try {
        //                Assertions.assertThat(h.check(null, PASSWORD, s)).isTrue();
        //            } catch (Exception e) {
        //                throw new RuntimeException(e);
        //            }}
        //        );
    ...
    public void PBKDF2HasherCLI_Should_ComputeHash() throws InvalidAlgorithmParameterException, UnsupportedEncodingException {
        ...
        try {
            ...
            System.setOut(ps);
            PBKDF2Hasher.main(Arrays.array("--encrypt", "--unsafe", new String(PASSWORD)));
    ...
    public void PBKDF2HasherCLI_Should_CheckHash() throws InvalidAlgorithmParameterException, UnsupportedEncodingException {
        ...
        try {
            PrintStream ps = new PrintStream(baos);
            System.setOut(ps);
            PBKDF2Hasher.main(Arrays.array("--check", (String) crypts()[0][0], "--unsafe", new String(PASSWORD)));
        } catch( ExitTrappedException e ) {
        } finally {
            enableSystemExitCall() ;
            System.out.flush();
