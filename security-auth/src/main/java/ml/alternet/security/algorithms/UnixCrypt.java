 ...
 *
 * modified April 2001
 * @author Iris Van den Broeke
 * @author Daniel Deville
 *
...
public class UnixCrypt {
    ...
    /**
     * Encrypts into crypt (Unix) code.
     *
     * @param salt the salt to be used
     * @param keyword the key to be encrypted
     * @return the encrypted bytes (without the salt)
     */
    public static byte[] encrypt(int salt, long keyword) {
        long constdatablock = 0L; /* encryption constant */
        byte[] cryptresult = new byte[11]; /* encrypted result */
        long[] KS = des_setkey(keyword);
        long rsltblock = des_cipher(constdatablock, salt, 25, KS);

        cryptresult[10] = (byte) ((((int) rsltblock) << 2) & 0x3f);
        rsltblock >>= 4;
        for (int i = 10; --i >= 0;) {
            cryptresult[i] = (byte) (((int) rsltblock) & 0x3f);
            rsltblock >>= 6;
        }
        return cryptresult;
    }
